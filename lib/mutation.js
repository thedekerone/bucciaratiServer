  ...
  addProductToBag: async (root, { productID, userID }) => {
    ...
    try {
      ...
      await db
        ...
        .updateOne(
          { _id: ObjectID(userID) },
          { $addToSet: { bag: { product: ObjectID(productID), quantity: 1 } } }
        )
    } catch (err) {
      console.log(err)
    }
  ...
  removeProductFromBag: async (root, { productID, userID }) => {
    ...
    try {
      ...
      product = await db
        ...
        .findOne({ _id: ObjectID(productID) })
      user = await db.collection('user').findOne({ _id: ObjectID(userID) })
      updatedUser = user.bag.filter((e, index) => {
        getIndex = index
        return e != productID
      })
      user.quantity.splice(getIndex, 1)
      // console.log(updatedUser.length);
      // user.bag.map((e) => console.log(e != productID));
      console.log(user.quantity)
      updateQuantity = user.quantity
      // console.log(productID);
      if (!user || !product) throw new Error('no hay brand o producto')
      await db
        .collection('user')
        .updateOne(
          { _id: ObjectID(userID) },
          { $set: { bag: updatedUser, quantity: updateQuantity } }
        )
    } catch (err) {
      console.log(err)
    }
